...
* **(Celery) Worker**
  * A service monitoring a Celery task queue that performs data ingestion and/or normalization on a given artifact (see [workers](https://github.com/baytemp/worker/tree/master/cucoslib/workers)).
* **(Celery) Task**
...
* **Selinon (Dispatcher)**
  * An implementation above Celery that helps us model Celery task flows in simple YAML configuration foles (see [worker configurations](https://github.com/baytemp/worker/tree/master/cucoslib/dispatcher)).
* **Scan Results**
