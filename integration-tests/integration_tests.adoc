...
By default, the system running on the localhost is tested. The <tests?> are not containerized currently hence they start and stop Fabric8 Analytics multiple times. Thus the following prerequisites need to be met:
