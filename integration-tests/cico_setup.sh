...
load_jenkins_vars() {
    if [ -e "jenkins-env" ]; then
        cat jenkins-env \
          | grep -E "(F8A_API_URL|F8A_API_V2_URL|OSIO_AUTH_SERVICE|F8A_JOB_API_URL|F8A_GREMLIN_URL|F8A_THREE_SCALE_PREVIEW_URL|F8A_3SCALE_URL|F8A_BACKBONE_API_URL|F8A_SERVICE_ID|F8A_GEMINI_API_URL|F8A_LICENSE_SERVICE_URL|RECOMMENDER_API_TOKEN|RECOMMENDER_REFRESH_TOKEN|THREE_SCALE_PREVIEW_USER_KEY|REGISTERED_USER_UUID|SNYK_TOKEN|JENKINS_URL|GIT_BRANCH|GIT_COMMIT|BUILD_NUMBER|ghprbSourceBranch|ghprbActualCommit|BUILD_URL|ghprbPullId)=" \
          | sed 's/^/export /g' \
          > ~/.jenkins-env
        source ~/.jenkins-env
    fi
